...
import { getOrderSummaryServerToken } from "../auth/authToken";
...
export class ServerSummaryBaseQuery {
  ...
  private async fetchInternal<T>(
    ...
    try {
      ...
      if (headers)
        Object.assign(finalHeader, headers as Record<string, string>);
  ...
  async fetch<T>(options: FetchOptions): Promise<ServerResult> {
    ...
    if (result.error?.status === 401 && !options.skipReauth) {
      ...
      const token = await getOrderSummaryServerToken(true);
...
export const serverSummaryBaseQuery = new ServerSummaryBaseQuery();
